...
const ListaPagamento = ({ dadosPaciente }) => {
    ...
    const agrupaPagamento = async (data) => {
        return await data?.reduce((acc, current) => {
            const { id_pagamento, nr_parcela, valor, dt_vencimento } = current
            ...
            acc[id_pagamento].parcelas.push({
                nr_parcela,
                valor,
                dt_vencimento
            })
                            ...
                            <div className="border rounded-lg shadow overflow-hidden">
                                <table className="min-w-full transition-all duration-300 ease-in-out">
                                    ...
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {loading ? <p>Loadnig...</p> : pagamentoAgrupado?.map((data, index) => (
                                            <>
                                                <tr key={index}>
                                                    <td
                                                        onClick={() => toggleParcelas(data?.id_pagamento)}
                                                        className="px-6 py-4 whitespace-nowrap text-sm text-purple-900 font-bold cursor-pointer">
                                                        {data.id_pagamento}
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-purple-900 font-bold">{data.total_pagamento}</td>
                                                </tr>
                                                {expandedPayment === data?.id_pagamento && (
                                                    <>
                                                        {/* <thead name="table2" className="bg-purple-800 dark:bg-purple-700"> */}
                                                        <tr for="table2" className="text-gray-500 text-left bg-gray-300 font-medium cursor-pointer">
                                                            <th scope="col" className="px-6 py-3"></th>
                                                        ...
                                                        {data?.parcelas?.map((parcela, index) => (
                                                            // <tbody name="table2">
                                                            <tr for="table2" key={data.id_pagamento + data.nr_parcela}>
                                                                <td className=" px-6 py-4 whitespace-nowrap text-sm text-success font-bold">
                                                                    <HiArrowNarrowRight />
                                                                </td>
